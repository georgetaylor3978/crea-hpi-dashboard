...
const btnExportTwitter = document.getElementById('btnExportTwitter');
...
function populateControls() {
    ...
    btnExportTwitter.disabled = false;
}
...
function exportForTwitter() {
    ...
    ctx.textAlign = 'left';
    ctx.fillText(chartTitle.textContent, 32, 48);

    // Subtitle
    ctx.fillStyle = '#8b95b0';
    ctx.font = '400 13px Inter, sans-serif';
    const dateRange = `${dateFrom.value || 'All'} → ${dateTo.value || 'All'}`;
    ctx.fillText(`CREA MLS® HPI  ·  ${dateRange}`, 32, 68);

    // Draw the chart into the export canvas
    const chartImage = mainChart.toBase64Image('image/png', 1);
    const img = new Image();
    img.onload = function () {
        // Draw chart area (with padding)
        const chartX = 24, chartY = 90;
        const chartW = W - 48, chartH = H - 130;
        ...
        ctx.fillText('Data: CREA MLS® Home Price Index  |  Generated by CREA HPI Dashboard', W / 2, H - 14);

        // Download
        const link = document.createElement('a');
        const location = locationSelect.value.replace(/\s+/g, '_');
        const prop = propertyType.value;
        link.download = `CREA_HPI_${location}_${prop}_${new Date().toISOString().slice(0, 10)}.png`;
        link.href = exportCanvas.toDataURL('image/png');
        link.click();

        showToast('Chart exported for Twitter (1200×675)', 'success');
    };
    img.src = chartImage;
}
